...
function openChampionsDetails(key){
  const [home,away]=String(key||"").split("|"),match=allChampionsMatches().find(item=>item.home===home&&item.away===away),dialog=document.getElementById("match-detail-dialog"),content=document.getElementById("match-detail-content");if(!match||!dialog||!content)return;
  const local=championsLocal(match),state=championsMatchState(match),score=Number.isInteger(match.homeScore)&&Number.isInteger(match.awayScore),round=championsData.rounds.find(item=>(item.matches||[]).includes(match));
  content.innerHTML=`<header class="detail-score"><p>UEFA Champions League · Jornada ${round?.round||match.round||"–"}</p><span class="status ${state}">${championsStatus(match)}</span><div><strong>${championsEscape(match.home)}</strong><b>${score?`${match.homeScore} – ${match.awayScore}`:"VS"}</b><strong>${championsEscape(match.away)}</strong></div><small>${championsEscape(local.date)} · ${local.time}</small></header><section class="detail-section"><div class="detail-title"><h3>Información del encuentro</h3><span>Datos oficiales UEFA</span></div><div class="referee-box"><span>Competición: UEFA Champions League</span><span>Fase: Liga · Jornada ${round?.round||match.round||"–"}</span><span>Estado: ${championsStatus(match)}</span>${match.referee?`<span>Árbitro: ${championsEscape(match.referee)}</span>`:`<span>Árbitro: Pendiente de designación oficial</span>`}</div></section><section class="detail-section"><div class="detail-title"><h3>Alineaciones y estadísticas</h3><span>Pendiente de UEFA</span></div><p class="detail-pending">Aparecerán aquí cuando estén disponibles en la fuente oficial. No se mostrarán datos inventados.</p></section><div class="card-actions"><a class="btn-link" href="${championsCalendarUrl(match)}" target="_blank" rel="noopener noreferrer">Añadir al calendario</a></div>`;
  dialog.showModal();
}
