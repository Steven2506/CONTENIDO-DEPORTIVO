...
function championsMatchCard(match){
  const local=match.iso?{date:new Intl.DateTimeFormat("es-ES",{weekday:"short",day:"numeric",month:"short",timeZone:window.WolfTimezone?.get()||"Europe/Madrid"}).format(new Date(match.iso)),time:new Intl.DateTimeFormat("es-ES",{hour:"2-digit",minute:"2-digit",hour12:false,timeZone:window.WolfTimezone?.get()||"Europe/Madrid"}).format(new Date(match.iso))}:{date:match.date,time:match.time};
  ...
  return `<article class="match-card champions-match" data-teams="${championsEscape(match.home)}|${championsEscape(match.away)}"><div class="match-meta"><span>${championsEscape(local.date)} · ${championsEscape(local.time)}</span><span class="status ${finished?"finished":""}">${finished?"FINAL":championsEscape(local.time)}</span></div><div class="score-teams"><strong>${championsEscape(match.home)}</strong><span class="big-score">${hasScore?`${match.homeScore}<i>–</i>${match.awayScore}`:"VS"}</span><strong>${championsEscape(match.away)}</strong></div><div class="card-actions">${calendar}</div></article>`;
}
