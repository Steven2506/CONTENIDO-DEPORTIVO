...
function fullMatch(match){const state=matchState(match),hasScore=Number.isInteger(match.homeScore)&&Number.isInteger(match.awayScore);return `<article class="match-card scoreboard-card ${state}" data-teams="${match.home}|${match.away}"><div class="match-meta"><span>${match.date} · ${match.time}</span><span class="status ${state}">${statusLabel(match)}</span></div><div class="score-teams"><strong>${match.home}</strong><span class="big-score">${hasScore?`${match.homeScore}<i>–</i>${match.awayScore}`:"VS"}</span><strong>${match.away}</strong></div>${state==="live"?`<p class="live-message"><span class="live-dot"></span> Actualización del marcador en curso</p>`:""}${state==="finished"&&!hasScore?'<p class="sync-note">Resultado pendiente de sincronización con la fuente oficial.</p>':""}<p>📍 ${match.venue}</p><div class="referee-box"><strong>Designación arbitral</strong><span>Árbitro: ${match.referee||"Pendiente de publicación oficial"}</span><span>VAR: ${match.var||"Pendiente de publicación oficial"}</span></div><div class="card-actions"><button class="btn-link share-event" data-share="${match.home} vs ${match.away} · ${match.date}, ${match.time}">Compartir</button><a class="btn-link" href="${calendarUrl(match)}" target="_blank" rel="noopener noreferrer">Añadir al calendario</a></div></article>`;}
