function findActiveFootballRound(){
  ...
  const future=rounds.map(item=>({round:item.round,time:Math.min(...item.matches.map(match=>match.iso?new Date(match.iso).getTime():Infinity).filter(time=>Number.isFinite(time)&&time>=now-3*3600000))})).filter(item=>item.time>=now-3*3600000).sort((a,b)=>a.time-b.time)[0];
...
function matchState(match){
  ...
  if(match.state==="live"){
    ...
    if((staleSecondHalf||sinceKickoff>180*60*1000)&&hasScore)return "finished";
    if(staleSecondHalf||sinceKickoff>180*60*1000)return "pending";
    return "live";
  }
  const inferred=inferredKickoffState(match);if(inferred)return inferred;
  if(match.iso&&match.state!=="postponed"&&Date.now()-new Date(match.iso).getTime()>=180*60*1000)return hasScore?"finished":"pending";
...
function localMatchSchedule(match){
  if(!match.iso)return {date:match.date,time:match.time,short:shortDate(match.date)};
  const value=new Date(match.iso),zone=window.WolfTimezone?.get()||"Europe/Madrid";
  const date=new Intl.DateTimeFormat("es-ES",{weekday:"long",day:"numeric",month:"long",timeZone:zone}).format(value).replace(/^./,letter=>letter.toUpperCase());
  const time=new Intl.DateTimeFormat("es-ES",{hour:"2-digit",minute:"2-digit",hour12:false,timeZone:zone}).format(value);
  const short=new Intl.DateTimeFormat("es-ES",{weekday:"short",day:"numeric",timeZone:zone}).format(value).replace(".","");
  return {date,time,short};
}
function statusLabel(match){const state=matchState(match);if(state==="live"){if(match.state!=="live"){const elapsed=Math.max(0,Math.floor((Date.now()-new Date(match.iso).getTime())/60000));if(elapsed<50)return `${Math.min(45,elapsed+1)}’`;if(elapsed<65)return "DESCANSO";return `${Math.min(90,46+elapsed-65)}’`;}if(match.period==="HalfTime")return "DESCANSO";const elapsed=match.periodStart?Math.max(0,Math.floor((Date.now()-new Date(match.periodStart).getTime())/60000)):null;if(match.period==="FirstHalf"&&elapsed!==null&&elapsed>=55)return "DESCANSO";const base=match.periodBase||0,rawMinute=Number(match.minute),providerMinute=Number.isFinite(rawMinute)?(rawMinute>base?rawMinute:base+rawMinute):null,minute=elapsed!==null&&elapsed<=60?base+elapsed+1:providerMinute;if(!minute)return "EN JUEGO";if(match.period==="FirstHalf"&&minute>45)return `45+${minute-45}’`;if(match.period==="SecondHalf"&&minute>90)return `90+${minute-90}’`;return `${minute}’`;}if(state==="finished")return "FINAL";if(state==="pending")return "POR CONFIRMAR";if(state==="rescheduled")return "REPROGRAMADO";if(state==="postponed")return "APLAZADO";return localMatchSchedule(match).time;}
function matchKey(match){return `${match.home}|||${match.away}`;}
function compactScore(match){const state=matchState(match),local=localMatchSchedule(match);return `<article class="score-chip ${state} match-detail-trigger" tabindex="0" role="button" data-match-key="${escapeHtml(matchKey(match))}" data-teams="${match.home}|${match.away}" aria-label="Ver ficha de ${match.home} contra ${match.away}"><div class="score-chip-top"><span>${local.short}</span><strong>${statusLabel(match)}</strong></div><div><span>${match.home}</span><b>${scoreValue(match.homeScore)}</b></div><div><span>${match.away}</span><b>${scoreValue(match.awayScore)}</b></div></article>`;}
function fullMatch(match){const state=matchState(match),local=localMatchSchedule(match),hasScore=Number.isInteger(match.homeScore)&&Number.isInteger(match.awayScore),calendarAction=match.iso?`<a class="btn-link" href="${calendarUrl(match)}" target="_blank" rel="noopener noreferrer">Añadir al calendario</a>`:'<span class="calendar-pending">Calendario disponible cuando se confirme el horario</span>',scheduleNotice=state==="rescheduled"?`<p class="rescheduled-note"><strong>↻ Partido reprogramado</strong><span>${match.scheduleNote||"Nueva fecha y hora confirmadas"}: ${local.date} a las ${local.time}.</span></p>`:"";return `<article class="match-card scoreboard-card ${state} match-detail-trigger" tabindex="0" role="button" data-match-key="${escapeHtml(matchKey(match))}" data-teams="${match.home}|${match.away}" aria-label="Ver ficha de ${match.home} contra ${match.away}"><div class="match-meta"><span>${local.date} · ${local.time}</span><span class="status ${state}">${statusLabel(match)}</span></div><div class="score-teams"><strong>${match.home}</strong><span class="big-score">${hasScore?`${match.homeScore}<i>–</i>${match.awayScore}`:"VS"}</span><strong>${match.away}</strong></div>${state==="live"?`<p class="live-message"><span class="live-dot"></span> En directo</p>`:""}${state==="finished"&&!hasScore?'<p class="sync-note">Resultado pendiente de sincronización con la fuente oficial.</p>':""}${scheduleNotice}<p>📍 ${match.venue}</p><div class="referee-box"><strong>Designación arbitral</strong><span>Árbitro: ${match.referee||"Pendiente de publicación oficial"}</span><span>VAR: ${match.var||"Pendiente de publicación oficial"}</span></div><p class="detail-hint">Toca la ficha para ver alineaciones y estadísticas →</p><div class="card-actions"><button class="btn-link share-event" data-share="${match.home} vs ${match.away} · ${local.date}, ${local.time}">Compartir</button>${calendarAction}</div></article>`;}
...
function playerRows(players=[]){return players.map(player=>`<li><b>${player.number||"–"}</b><span>${escapeHtml(player.name)}</span></li>`).join("");}
function lineupColumn(team,data){if(!data?.starters?.length)return `<section class="lineup-team"><h4>${escapeHtml(team)}</h4><p class="detail-pending">Alineación pendiente de confirmación oficial.</p></section>`;return `<section class="lineup-team"><h4>${escapeHtml(team)} <small>${escapeHtml(data.formation||"")}</small></h4>${data.manager?`<p class="manager">Entrenador: ${escapeHtml(data.manager)}</p>`:""}<h5>Titulares</h5><ol>${playerRows(data.starters)}</ol>${data.substitutes?.length?`<details class="lineup-subs"><summary>Suplentes (${data.substitutes.length})</summary><ol>${playerRows(data.substitutes)}</ol></details>`:""}</section>`;}
function incidentList(details){const incidents=details?.events||[];if(!incidents.length)return '<p class="detail-pending">Los goles, tarjetas rojas y otros momentos decisivos aparecerán aquí durante el encuentro.</p>';const labels={goal:"⚽ Gol",red:"🟥 Expulsión",yellow:"🟨 Tarjeta amarilla",substitution:"🔄 Sustitución"};return `<ul class="incident-list">${incidents.map(event=>`<li class="${event.type}"><strong>${escapeHtml(event.minute)}’</strong><span>${labels[event.type]||"• Incidencia"} · ${escapeHtml(event.player)} <small>${escapeHtml(event.team||"")}</small></span></li>`).join("")}</ul>`;}
function openMatchDetails(key){
  ...
  const local=localMatchSchedule(match),details=match.details||{},stats=details.stats||{},probability=prediction(match),hasScore=Number.isInteger(match.homeScore)&&Number.isInteger(match.awayScore);
  content.innerHTML=`<header class="detail-score"><p>${escapeHtml(local.date)} · ${escapeHtml(local.time)}</p><span class="status ${matchState(match)}">${statusLabel(match)}</span><div><strong>${escapeHtml(match.home)}</strong><b>${hasScore?`${match.homeScore} – ${match.awayScore}`:"VS"}</b><strong>${escapeHtml(match.away)}</strong></div><small>📍 ${escapeHtml(match.venue)}</small></header><section class="detail-section"><div class="detail-title"><h3>Probabilidad del partido</h3><span>Estimación WOLFGAMES</span></div><div class="probabilities">${probabilityBar(match.home,probability.home,"home")}${probabilityBar("Empate",probability.draw,"draw")}${probabilityBar(match.away,probability.away,"away")}</div><p class="method-note">Estimación orientativa basada en localía y posición liguera; no es una cuota de apuestas ni un dato oficial.</p></section><section class="detail-section"><div class="detail-title"><h3>Alineaciones</h3><span>${details.source||"Fuente oficial LALIGA"}</span></div><div class="lineups-grid">${lineupColumn(match.home,details.lineups?.home)}${lineupColumn(match.away,details.lineups?.away)}</div></section><section class="detail-section"><div class="detail-title"><h3>Estadísticas</h3><span>${details.updatedAt?`Actualizado ${escapeHtml(details.updatedAt)}`:"Disponibles al comenzar"}</span></div><div class="stats-list">${statRow("Posesión",stats.home?.possession,stats.away?.possession,"%")} ${statRow("Pases",stats.home?.passes,stats.away?.passes)} ${statRow("Tiros",stats.home?.shots,stats.away?.shots)} ${statRow("Tiros a puerta",stats.home?.shotsOnTarget,stats.away?.shotsOnTarget)} ${statRow("Tarjetas amarillas",stats.home?.yellowCards,stats.away?.yellowCards)} ${statRow("Tarjetas rojas",stats.home?.redCards,stats.away?.redCards)}</div></section><section class="detail-section"><div class="detail-title"><h3>Incidencias</h3><span>En directo</span></div>${incidentList(details)}</section>`;
  dialog.showModal();
}
