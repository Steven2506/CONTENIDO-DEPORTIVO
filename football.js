...
document.addEventListener("DOMContentLoaded",()=>{
  ...
  renderFootball();
  setInterval(checkForDataUpdate,60000);
});
...
function statusLabel(match){const state=matchState(match);if(state==="live")return `${match.minute||"EN JUEGO"}${match.minute?"’":""}`;if(state==="finished")return "FINAL";if(state==="rescheduled")return "REPROGRAMADO";if(state==="postponed")return "APLAZADO";return match.time;}
